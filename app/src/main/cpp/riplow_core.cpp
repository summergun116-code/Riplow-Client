...
extern "C" JNIEXPORT jstring JNICALL Java_com_riplow_client_NativeBridge_version(JNIEnv* env,jobject){
    return env->NewStringUTF(riplow::core_version());
}
extern "C" JNIEXPORT jstring JNICALL Java_com_riplow_client_NativeBridge_nativeModuleSummary(JNIEnv* env,jobject){
    return to_jstring(env,riplow::modules().summary());
}
extern "C" JNIEXPORT jboolean JNICALL Java_com_riplow_client_NativeBridge_nativeToggleModule(JNIEnv* env,jobject,jstring id){
...
extern "C" JNIEXPORT jstring JNICALL Java_com_riplow_client_NativeBridge_nativeDiagnostics(JNIEnv* env,jobject){
    const auto frame=riplow::performance().metrics();
    std::string output="Modules: "+std::to_string(riplow::modules().enabled_count())+"/"+std::to_string(riplow::modules().size());
