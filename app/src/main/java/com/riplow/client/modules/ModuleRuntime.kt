...
object ModuleRuntime {
    ...
    fun status(context: Context, prefs: android.content.SharedPreferences, module: ModuleDefinition): String {
        return when (module.id) {
            "clock" -> {
                val fallback = ModuleSetting("format", "Format", "", listOf("12-hour", "24-hour"))
                val format = ModuleManager.setting(prefs, module.id, module.settings.firstOrNull() ?: fallback)
                SimpleDateFormat(if (format.contains("24")) "HH:mm" else "h:mm a", Locale.getDefault()).format(Date())
            }
            "session_timer" -> {
                ...
                val s = total % 60L
                if (prefs.getString("module_setting_session_timer_format", "MM:SS")?.startsWith("HH") == true)
            ...
            "battery_guard" -> {
                ...
                if (level >= 0) "Battery $level%" else "Battery unavailable"
            }
            "performance_profile", "fps_boost_profile" -> {
                val fallback = ModuleSetting("profile", "Profile", "", listOf("Balanced"))
                "Profile " + ModuleManager.setting(prefs, module.id, module.settings.firstOrNull() ?: fallback)
            }
            "network_diagnostics", "ping", "jitter_monitor", "packet_loss", "connection_status",
            "transport_inspector", "session_transport" -> {
                try {
                    NativeBridge.nativeDiagnostics().lineSequence().lastOrNull() ?: "Waiting for network samples"
                } catch (_: Throwable) {
