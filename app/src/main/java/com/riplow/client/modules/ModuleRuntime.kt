...
object ModuleRuntime {
    ...
    fun status(
        ...
        return when (module.id) {
            "clock" -> {
                val format = ModuleManager.setting(prefs, module.id, module.settings.first())
                SimpleDateFormat(
            ...
            "session_timer" -> {
                ...
                val s = total % 60L
                val format = prefs.getString("module_setting_session_timer_format", "MM:SS") ?: "MM:SS"
                if (format.startsWith("HH")) "%02d:%02d:%02d".format(Locale.US, h, m, s)
            ...
            "performance_profile" -> {
                ...
                "Profile " + ModuleManager.setting(prefs, module.id, setting)
            }
            "ping", "network_diagnostics", "jitter_monitor", "packet_loss", "connection_status" ->
                NativeBridge.nativeDiagnostics().lineSequence().lastOrNull { it.isNotBlank() }
                    ?: "Waiting for Bedrock telemetry"
            "quick_launch" -> {
                val install = MinecraftCompatibility.detect(context)
                if (install.installed) "Minecraft " + install.versionName else "Minecraft not detected"
            }
            "module_config" -> "JSON config ready"
            else -> "Minecraft integration required"
        }
    }
}
